...
function lifecyclePolicyBlock(): string {
	return [
		...
		"- Two kinds of blocker, handled differently. A blocker only YOU can clear (a decision that is the user's to make, a missing credential, an approval) is reported the first time you meet it: finish everything the objective still allows without that answer, then call update_goal({status: \"blocked\"}) with the question, and ask. Do not spend turns re-attempting something only the user can settle. A technical blocker you might still solve is different: report it with update_goal({status: \"blocked\"}) only after the SAME blocker recurs on three consecutive goal turns, and keep trying concrete next steps before that. A user pause is a distinct state controlled by the user (/goal-pause, Esc).",
		"- Do the reachable work first. A pending question does not excuse leaving untouched work that does not depend on its answer; blocking with that work still available wastes the time you were given. Equally, do not manufacture busywork to look productive - if nothing is left that the answer does not govern, block and ask.",
		"- A blocked goal stops receiving continuations, so the question reaches the user rather than being buried under further turns. It resumes when the user answers.",
